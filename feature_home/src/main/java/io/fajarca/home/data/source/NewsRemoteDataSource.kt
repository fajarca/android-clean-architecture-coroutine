...
class NewsRemoteDataSource (private val newsService: NewsService) : RemoteDataSource() {
    ...
    suspend fun getNews(dispatcher: CoroutineDispatcher, country : String, page : Int, pageSize : Int): Result<NewsDto> {
        return safeApiCall(dispatcher) { newsService.getNews(country, page, pageSize) }
    }
